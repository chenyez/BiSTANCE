...
def convert_data_to_ids(tokenizer, target, text, label, config):
    ...
    for tar, sent in zip(target, text):
        ...
        # print(bk)
    print(200*"#")
    print("To be encoded[0]:",concat_sent[0])
    print("To be encoded[-1]:",concat_sent[-1])
...
def data_helper_bert(x_train_all,x_val_all,x_test_all,model_select,config):
    ...
    print("x_val:",x_val[:3],len(x_val))
    print("y_val:",y_val[:3],len(y_val))
...
def data_helper_bert_eval(x_test_all,model_select,config):
...
def data_loader(x_all, batch_size, model_select, mode):
    ...
    # print("x_all['gt_label']:",x_all['gt_label'])
