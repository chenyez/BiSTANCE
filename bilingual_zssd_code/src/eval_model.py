...
def run_classifier():
    ...
    parser.add_argument('-name', '--name', type=str, default='EZ, mixed', help='setting name', required=False)
...
#     random_seeds = [0,1,2,3,4,42]
    ...
    for seed in random_seeds:    
        ...
        # prepare for model
        loader, gt_label = dh.data_helper_bert_eval(x_test_all, model_select, config)
        testloader = loader
        y_test= gt_label
        ...
        ###保存performance
        best_test_loss_result = result_one_sample_test
        best_test_loss_result[0][0]=args['name']
        results_df = pd.DataFrame(best_test_loss_result)    
        print("results_df are:",results_df.head())
        results_df.to_csv('./eval_results.csv',index=False, mode='a', header=False)    
        print('./eval_results.csv save, done!')
